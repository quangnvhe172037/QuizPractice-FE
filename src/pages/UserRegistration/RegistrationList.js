...
function UserRes() {
  ...
  const displayPaginatedUserPayments = (payments) => {
    return payments.slice(offset, offset + itemsPerPage).map((userPayment) => (
      <div key={userPayment.billID} className="course-card col-md-3">
        <div className="post">
          <Link
            to={`/subject/${userPayment.subject.subjectID}`}
            state={userPayment.subject.subjectID}
          >
          ...
          <p>
            Purchase Date:{" "}
            {format(new Date(userPayment.purchase_date), "dd-MM-yyyy")}
          </p>
        </div>
      </div>
    ));
